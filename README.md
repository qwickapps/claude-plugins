...
Official plugin marketplace for QwickApps development with Claude Code. Three plugins covering the full development lifecycle: SDLC discipline, tech stack guidance, and UX design enforcement.
...
/plugins install qwickapps-sdlc           # SDLC workflows and quality gates
/plugins install qwickapps-dev-guide      # Tech stack setup and patterns
/plugins install qwickapps-ux-design      # Design system enforcement
```
...
**Rules**: Use only `@qwickapps/react-framework` components. Use only `--theme-*` CSS variables for colors. If a component is missing, extend the framework first.
...
claude-plugins/
  ...
  plugins/
    qwickapps-sdlc/           # SDLC workflows and discipline
    ...
    qwickapps-dev-guide/      # Tech stack guidance
      skills/                 # 4 skills (use-stack + 3 per-package)
      scripts/                # Validation utilities
    qwickapps-ux-design/      # Design system enforcement
      skills/                 # 3 skills
...
1. **Start work** with `qwickapps-sdlc` commands (`/feature`, `/bug`, etc.) to create issues and follow disciplined workflows
2. **Set up the stack** with `qwickapps-dev-guide` skills when building or extending a QwickApps product
3. **Build the UI** with `qwickapps-ux-design` enforcing design system consistency throughout

The SDLC plugin provides the process. The dev guide provides the architecture. The UX design plugin provides the guardrails.

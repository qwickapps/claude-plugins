...
description: QwickApps worktree workflow that configures the SDLC creating-worktree skill. Sets QwickApps-specific variables (script path, worktree prefix, pnpm) so the generic SDLC skill uses the correct create-worktree.sh script, .env copying, and dependency installation. Trigger phrases include "create a worktree", "set up a worktree", "start a new branch", "isolate this work", or before executing any implementation plan in qwickapps.
---

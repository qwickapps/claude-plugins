...
**Load skill:** `cloud:planning-infrastructure`
...
**Dispatch agent:** `cloud:infra-planner`
...
```bash
if [ -f ~/qwickapps-env.sh ]; then
  source ~/qwickapps-env.sh
  echo "Environment loaded: domain=$MY_DOMAIN, region=$OCI_REGION"
else
  echo "Environment file ~/qwickapps-env.sh not found."
  echo "Create it from the template in the service-catalog reference."
fi
```
If missing: Show the user the template from `${CLAUDE_PLUGIN_ROOT}/references/service-catalog.md` and ask them to fill in their values. **Do not write `~/qwickapps-env.sh` without explicit user confirmation.** Present the template content and ask: "Should I create this file at ~/qwickapps-env.sh with these placeholder values for you to fill in?"
...
**Load skill:** `cloud:provisioning-oci`
...
**Load skill:** `cloud:configuring-services`
...
**Load skill:** `cloud:setting-up-dns`
...
**Load skill:** `cloud:setting-up-free-services`
...
After health checks pass, generate `~/qwickapps-topology.yml` with the actual provisioned state. This file is the source of truth for future infrastructure operations.

```bash
cat > ~/qwickapps-topology.yml << EOF
# QwickApps Infrastructure Topology
# Auto-generated by cloud plugin on $(date -u +%Y-%m-%dT%H:%M:%SZ)
# Updated on each provisioning change. Do not commit to git.

domain: $MY_DOMAIN
oci_region: $OCI_REGION
provisioned: $(date -u +%Y-%m-%d)

vms:
  # Populate one entry per VM in the allocation:
  oci-main:
    ...
    software: caprover
    dns:
      - apps.$MY_DOMAIN (proxied)
      - "*.apps.$MY_DOMAIN (dns-only)"
    ssh: ssh oci-main
    status: running

resources:
  total: { ocpus: 4, ram_gb: 24, storage_gb: 200 }
  used: { ocpus: <N>, ram_gb: <N>, storage_gb: <N> }
  remaining: { ocpus: <N>, ram_gb: <N>, storage_gb: <N> }

services:
  database: <neon|self-hosted|supabase|none>
  storage: <r2|b2|supabase-storage|none>
  redis: <upstash|none>
  email: <resend|none>
...
networking:
  ...
  subnet_ocid: <ocid>
EOF
chmod 600 ~/qwickapps-topology.yml
```

Populate each VM entry with the actual values from provisioning. Only include VMs that were created. The `resources` section must reflect actual totals. Present the file to the user before writing.

**This file is read by the infra-planner agent and all skills before making changes.** Future `/cloud-setup` runs, VM additions, or infrastructure changes read it to understand what exists.

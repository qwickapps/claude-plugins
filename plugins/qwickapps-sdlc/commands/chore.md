...
Load `qwickapps-sdlc:tracking-issues` to create a new GitHub issue with the label `chore`. If the user provided an existing issue number, link to it rather than creating a new one.
...
**CI/CD changes:**
- Load `qwickapps-sdlc:deploying` for context on the deployment pipeline
...
- Load `qwickapps-sdlc:writing-tests` if the cleanup touches behavior (e.g., removes a workaround, fixes a misnamed function)
...
Load `qwickapps-sdlc:creating-worktree` to set up an isolated workspace if the change involves code or configuration files. Follow WORKTREE-ENFORCEMENT.md: use the create-worktree.sh script.
